...
Using potentiometers the Arduino will be made into an EtchASketch.
...
Step 3: I connected the potentiometers to the Arduino. The left leg went to power, the right went to ground, and the middle went to pins A0 and A1.
...
How it works is the Arduino reads the two potentiometer values.
...
So a main difference from your typical EtchASketch is that this version the position of the potentiometers correlate to the canvas position.
Then the Arduino checks if a button was pressed to clear the screen.
It then sends the x value, y value, and button state to the Processing IDE.
...
The main challenge was the processing code because I was unfamiliar with that the most.
But luckily the documentation helped me out a lot.

To learn more about the process check out the Post Mortem video down below.

[![Post Mortem](https://img.youtube.com/vi/yUN5XKVlcZ4/0.jpg)](https://youtu.be/yUN5XKVlcZ4)
## Resources
